...
// 代入演算子、インクリメント演算子、デクリメント演算子のように、
// delete演算子はプロパティを削除するという副作用のために使われるのが一般的である
// delete演算子の値が使われることはまずない
...
// 削除されたプロパティや配列要素には、単にundefinedが設定されるだけではない
// プロパティが削除されると、プロパティは存在しなくなる
// 存在しないプロパティを読み出すとundefinedが返されるので、これだけでは区別できないですが、
// in演算子(4.9.3章)を使うことでプロパティが存在するかどうか区別できる
// 配列の要素を削除すると、配列に「穴」があくだけで、配列の長さ自体は変化しない
// deleteを使った配列は疎な配列(7.3章)になる

// delete演算子のオペランドには、左辺値を指定する
// もしも左辺値ではない場合、delete演算子は何も処理せず、trueを返す
// 左辺値を指定した場合は、delete演算子は指定された左辺値を削除しようとする
// 指定された左辺値が削除できた場合には、delete演算子はtrueを返す
// しかし、すべてのプロパティが削除できるわけではない
// 再定義不可のプロパティ(14.1章)は削除できない

// strictモードでは、変数や関数、関数パラメータを削除しようとすると、delete演算子はSyntaxErrorを発生させる
// delete演算子は、オペランドとしてプロパティアクセス式(4.4章)を指定したときにのみ動作する
// strictモードでは、再定義不可(削除不可)のプロパティを削除しようとすると、delete演算子はTypeErrorを発生させる
// 非strictモードでは、前述のような場合にも例外は発生せず、オペランドが削除できなかったことを示すために、delete演算子はfalseを返すだけである
...
// delete演算子については、後ほど6.4章で解説する

//////////////////////////////////////////////////////////////////////////////////////
// 4.13.5 await演算子
//////////////////////////////////////////////////////////////////////////////////////

// awaitは、JavaScriptで非同期プログラミングを簡単に記述できるように、ES2017で導入された
// この演算子を理解するには、13章を読む必要がある
// 簡単に説明しておくと、awaitはオペランドとして、非同期処理を表すPromiseオブジェクトを1つだけ取り出せる
// awaitは非同期処理が完了するのを待っているかのようにプログラムを動作させる
// (ただし、実際にはプログラムがブロックすることもなく、また、同時に行われているほかの非同期処理を止めることもない)
// await演算子の値はPromiseオブジェクトを満たした値になる
// なお、awaitはasyncキーワードを使って非同期と宣言された関数の中でしか使えない
// 詳細は13章で解説する
...
// void演算子は単項演算子である
// オペランドとしては任意の値が取れる
// void演算子は、あまり使われるものではない
// void演算子は、オペランド値を評価した後破棄し、undefinedを返すという特殊なものであるからである
// オペランド値を破棄するので、void演算子を使って意味のある場合は、オペランドが副作用を持つときだけである

// void演算子の実際の使用例はなかなか思いつかない
// 1つの例としては、値を返さない関数を定義するときに、関数本体が1つの式というアロー関数の簡略記法(8.1.3章)を使いたい場合である
// この後部んでは関数本体の式が評価され、その値が返される
// 副作用のためだけに式を評価し、式の値は返したくないのであれば、関数本体を中括弧で囲むのが一番カンタンな方法である
// しかし、別の方法として、次のようにvoid演算子を使う方法が考えられる
...
// カンマ(,)演算子は二項演算子である
// オペランドには任意の型の値を指定できる
// まず左辺のオペランドを評価し、次に右辺のオペランドを評価し、最後に右辺のオペランドの値を返す
...
// 左辺の式は評価されるが、値は捨てられる
// つまり、カンマ演算子を使って意味があるのは、左辺の式に副作用がある場合だけである
// 一般的に、過ｍな演算子が使われるのは、forループ(5.4.3章)で複数ループ変数を使うときである

// 以下の例の最初のカンマは、let文の構文の一部
// 2つ目のカンマがカンマ演算子
// カンマ演算子で、2つの式（i++とj--)を1つの式しか受け付けない文（forループ）中に記述している
...
// この章ではさまざまなトピックを学習した
// JavaScriptを学んでいく上で、もう一度読みたくなるものがたくさんあるはずである
// 本章のまとめとして、いくつかの重要なポイントだけ以下に記述する

// ・式はJavaScriptの語句にあたるものである
// ・すべての式はJavaScriptの値に評価される
// ・式は値を生成するだけでなく、変数への代入など副作用を持つことができる
// ・リテラルや変数参照、プロパティアクセス式などの単純な式と演算子を組み合わせることで、より大きな式を記述できる
// ・JavaScriptには、算術演算子や比較、論理演算、代入、ビット操作のための演算子が用意されている
// 　そのほか、三項演算子などさまざまな演算子も定義されている
// ・JavaScriptの+演算子は数値の加算にも文字列の結合にも使われる
// ・論理演算子の&&と||は特殊な「短絡」処理が行われ、オペランドが1つしか評価されない場合がある
// 　この処理はJavaScriptで広く使われているため、&&と||演算子の特殊な処理については理解しておく必要がある
